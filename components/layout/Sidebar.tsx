...
const NavItem: React.FC<NavItemProps> = ({ icon, label, isActive, as = 'a', onClick }) => {
  const commonClasses = `flex items-center w-full px-3 py-2.5 text-sm rounded-md transition-colors duration-200 ${isActive
...
const Sidebar: React.FC<SidebarProps> = ({ onLogout, isOpen, setIsOpen, activeView, onNavigate, onLoadDemoData, performanceMode, onTogglePerformanceMode }) => {
  return (
    <>
      {/* Overlay for mobile */}
      <div
      ...
      <aside className={`fixed top-0 left-0 w-[200px] h-full bg-background/80 backdrop-blur-xl flex flex-col p-4 border-r border-border z-40 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        ...
        <div className="mt-auto flex flex-col gap-1">
          <NavItem as="button" onClick={onTogglePerformanceMode} label={performanceMode ? "LookMaxx: Off" : "LookMaxx: On"} />
